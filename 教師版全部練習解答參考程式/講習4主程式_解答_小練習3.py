...
for po in position:
    balllist.append(box(pos=vector(-L*cos(th)*po, L*sin(th)*po-size/2, 0.6-po), axis = vector(-cos(th), sin(th), 0),
...
#2
    ...
    while ball.pos.x < 0.0 :
        ...
        a_friction = vector( - g*cos(th)*cos(th)*u, g*cos(th)*sin(th)*u, 0) 
        ball.v += (a + a_friction) * dt                                            
        ball.pos += ball.v * dt                                       
    print "時間=",t
#3
